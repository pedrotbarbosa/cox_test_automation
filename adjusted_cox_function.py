# -*- coding: utf-8 -*-
"""V2 Adjusted Cox Function.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1sJDoGwTZESEm6CbS8jgj5L8FOys5QDJH
...
# Upload file
#uploaded = files.upload()
df_map = pd.read_csv('imputed_df_map.csv')
...
Before fitting the Cox model, the dataset must undergo essential preprocessing steps to ensure proper model behavior and interpretable results.

 * First, the outcome variable must be binary. In this case, the column <code>outco_binary_outcome</code> is mapped to a new variable called <code>outcome_binary</code>, where values like <code>"Death"</code> are mapped to 1, and <code>"Discharged"</code> or <code>"Censored"</code> to 0.

 * Next, the time-to-event variable is calculated as the number of days between admission and outcome dates. Any rows with negative durations are excluded.

 * Additionally, to ensure clear labeling of hazard ratios, all variables that contain boolean values (i.e., <code>True</code> or <code>False</code>), even if stored as <code>float64</code> or <code>object</code>, are explicitly cast to the <code>category</code> data type. This ensures that one-hot encoding is correctly applied later in the model pipeline, generating interpretable variable names such as <code>comor_hypertensi_1.0</code> or <code>demog_healthcare_1.0</code>, where the suffix denotes the positive class compared to the baseline.

</dd>
"""

# Make a copy to keep the original df_map intact
df_cox_prep = df_map.copy()

# 1. Calculate the duration (time-to-event) in days and name the column 'duration_col'
...
# Calculate the difference and name the new column 'duration_col'
df_cox_prep['duration_col'] = (df_cox_prep['outco_date'] - df_cox_prep['dates_admdate']).dt.days

# Data Quality Check for the new duration column
negative_duration_count = (df_cox_prep['duration_col'] < 0).sum()
if negative_duration_count > 0:
    print(f"Warning: Found {negative_duration_count} rows with negative duration. Setting them to NaN.")
    df_cox_prep.loc[df_cox_prep['duration_col'] < 0, 'duration_col'] = np.nan

# 2. Binarize the outcome variable
df_cox_prep['outcome_binary'] = df_cox_prep['outco_binary_outcome'].map({
    "Death": 1, "Censored": 0, "Discharged": 0
})

# 3. Ensure that all columns with boolean values (even if typed as float or object) are treated as categorical
bool_like_columns = []

for col in df_cox_prep.columns:
    try:
        non_null_values = df_cox_prep[col].dropna()
        if non_null_values.isin([True, False]).all():
            bool_like_columns.append(col)
    except Exception:
        continue  # Skip columns that raise an error (e.g., datetime)

df_cox_prep[bool_like_columns] = df_cox_prep[bool_like_columns].astype('category')
...
# Time, outcome, and predictor variables
duration_col_dfmap = 'duration_col'  # Time variable
event_col_dfmap = 'outcome_binary'   # Binary outcome variable

cox_predictors_dfmap = [
    'demog_sex', 'demog_healthcare',
    'comor_hypertensi', 'comor_chrkidney', 'comor_liverdisease', 'comor_obesity', 'comor_chrkidney_stag',
    'comor_liverdisease_type'
]

# Run the Cox model
results_dfmap, cph_dfmap, df_cox_dfmap = execute_cox_model(
    df=df_cox_prep,
    duration_col=duration_col_dfmap,
    event_col=event_col_dfmap,
    predictors=cox_predictors_dfmap
)
...
# Choose the variable you want to visualize
variable_to_plot_dfmap = 'comor_hypertensi_1.0'
...
test_and_plot_schoenfeld(
    ...
    duration_col_dfmap,
    event_col_dfmap,
    covariate_to_plot=variable_to_plot_dfmap
)
...
* **Martingale Residuals**
  ...
  Martingale residuals are a diagnostic tool used to assess the functional form of covariates in a Cox proportional hazards model. They are particularly useful for checking whether each covariate has a linear relationship with the log-hazard. However, unlike residuals in linear regression, Martingale residuals are not centered at zero — they range from −∞ to 1 and are skewed, which can complicate interpretation, especially in categorical variables.
  ...
  * **Interpretation for Continuous Variables**
    ...
    * **Well-specified model**: The residuals should appear as a random cloud, and a smoothed curve (e.g., LOESS) should be roughly flat and close to zero.
    * **Poorly-specified model**: A systematic shape, such as a curve or a slope in the smoothed line, suggests the linearity assumption is violated — e.g., a U-shape could suggest the need for a quadratic term.

  * **Interpretation for Categorical Variables**

    Since the residuals are not centered at zero and can be highly skewed, box plots can be used for exploratory comparisons across categories. Caution is warranted here:
    * **Well-specified model**: The distribution of residuals across categories should not show systematic shifts.
    * **Poorly-specified model**: A category with consistently higher or lower residuals might suggest a misfit, though the lack of symmetry makes this less precise than with other residuals.
        * A higher median (closer to 1) might indicate underestimation of risk.
        * A lower median (strongly negative) might indicate overestimation.

    Because of these interpretive challenges, deviance residuals — which are transformations of Martingale residuals — are often preferred in practice for identifying outliers or assessing model fit, as they are approximately symmetric and centered.

  * **Outlier Detection**

    Martingale residuals can help identify subjects who deviate substantially from the model's predictions:
    * Residuals close to 1 indicate subjects who had an event earlier than expected (given low predicted risk).
    * Large negative residuals (e.g., -2 or lower) reflect subjects who survived much longer than predicted.
...
# Choose the categorical variable you want to inspect
variable_to_check_dfmap = 'comor_obesity_1.0'

print(f"Generating Martingale residual plot for the variable: '{variable_to_check_dfmap}'...")
...
plot_martingale_residuals_manual(
    ...
    duration_col_dfmap,
    event_col_dfmap,
    covariate_name=variable_to_check_dfmap
)
...
* **Deviance Residuals**
  ...
  Deviance residuals are a transformation of Martingale residuals designed to address some of their interpretive limitations. While Martingale residuals are bounded above by 1 and skewed to the left (not centered at zero), deviance residuals are approximately symmetric and centered, making them easier to interpret — especially when detecting outliers or checking model adequacy.

  * **Interpretation for Continuous Variables**

    Similar to Martingale residuals, deviance residuals can be plotted against a continuous covariate to assess functional form.
    * **Well-specified model**: The points should form a random scatter around the zero line, with no systematic shape. A LOESS curve should be flat and close to zero.
    * **Poorly-specified model**: A clear pattern (e.g., curvature or trend in the smoothed line) suggests that the covariate may not have a linear relationship with the log-hazard and might benefit from transformation or inclusion of higher-order terms.

  * **Interpretation for Categorical Variables**

    For categorical predictors, box plots of deviance residuals across groups provide a more interpretable diagnostic than Martingale residuals due to their improved symmetry.
    * **Well-specified model**: Residual distributions across categories should be roughly centered around zero and show similar spread.
    * **Poorly-specified model**: A systematic shift in the median or spread for one or more categories may suggest a misfit.
        * A category with consistently higher residuals might indicate underestimation of risk.
        * A category with consistently lower residuals might indicate overestimation.

  * **Outlier Detection**

    Because they incorporate both the direction and magnitude of model misspecification, deviance residuals are commonly used for identifying influential data points.
    * Large positive or negative residuals indicate subjects whose observed outcomes deviate significantly from what the model predicts.
    * Thresholds for flagging outliers vary, but extreme values (e.g., > |2|) often merit further investigation.
...
def plot_deviance_residuals_manual(cph_model, dataframe, duration_col, event_col, covariate_name):
    """
    Calculates and plots deviance residuals against a categorical covariate
    for a fitted Cox Proportional Hazards model.

    Args:
        cph_model (CoxPHFitter): The fitted CoxPH model.
        dataframe (pd.DataFrame): Data used to fit the model.
        duration_col (str): Name of the time-to-event column.
        event_col (str): Name of the event indicator column.
        covariate_name (str): Name of the categorical variable to analyze.
    """
    try:
        # Get event indicator and risk scores
        E = dataframe[event_col].values
        risk_scores = cph_model.predict_partial_hazard(dataframe).values
...
print(f"Generating Deviance residual plot for the variable: '{variable_to_check}'...")

# Execute the function
plot_deviance_residuals_manual(
    cph_model=cph,
    dataframe=df_cox,
...
print(f"Generating Deviance residual plot for the variable: '{variable_to_check_dfmap}'...")

# Execute the function
plot_deviance_residuals_manual(
    cph_model=cph_dfmap,
    dataframe=df_cox_dfmap,
    duration_col=duration_col_dfmap,
    event_col=event_col_dfmap,
    covariate_name=variable_to_check_dfmap
)
...
# Execute the function
calculate_c_index(cph_dfmap, df_cox_dfmap, duration_col_dfmap, event_col_dfmap)
...
# Define the time point you want to evaluate
# For example, 5-year survival (if your time unit is years) or 365 days (if your time unit is days).
time_to_evaluate = 10 # <--- IMPORTANT: Adjust this time to a relevant value for your data
...
# Choose a meaningful time point for calibration
calibration_time = 12 # <--- IMPORTANT: Adjust this time to a relevant value for your data

# Execute the function
with warnings.catch_warnings():
    warnings.simplefilter("ignore", ApproximationWarning)
    plot_calibration(cph_dfmap, df_cox_dfmap, duration_col_dfmap, event_col_dfmap, t=calibration_time)
...
# Define the time point you want to evaluate
time_to_evaluate = 20 #<--- IMPORTANT: Adjust this time to a relevant value for your data
...
# Execute the function
display_model_fit_metrics(cph_dfmap, df_cox_dfmap)
